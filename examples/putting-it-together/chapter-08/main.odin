...
make_camera :: proc() -> rt.Camera {
	...
	view_position := m.point(-2, 1.35, -2.75)
	target := m.point(-0.25, 0.85, 0)
	up := m.vector(0, 1, 0)
...
main :: proc() {
	...
	small_sphere.transform = m.mat4_translate(m.vector(-1.5, 0.33, -0.3)) * m.mat4_scale(0.33)
	...
	path := fs.get_absolute_path("/examples/putting-it-together/chapter-08/scene-with-shadows.ppm")
